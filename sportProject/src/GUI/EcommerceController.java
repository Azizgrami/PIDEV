...
public class EcommerceController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        List<Product> products = productService.afficherProduits();
...
imageView.setImage(image);
    ...
    // Créer une liste contenant le produit choisi
    List<Product> productList = new ArrayList<>();
    productList.add(product);

    // Créer un Panier avec la liste de produits et la quantité
    Panier panier = new Panier(1, productList);

    // Ajouter le produit au panier
    servicePanier.addProductToCart(panier, product);

    // Ajouter une logique supplémentaire ici, comme afficher un message de confirmation
    System.out.println("Produit choisi et ajouté au panier : " + product.getProductName());
}
