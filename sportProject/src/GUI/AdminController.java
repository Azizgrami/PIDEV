...
public class AdminController implements Initializable {
 ...
 try {
    ...
    @FXML
void handleCategoryBtn(ActionEvent event) {
    try {
        // Créer un nouveau stage pour l'interface "CategoryController"
        Stage categoryStage = new Stage();

        // Charger l'interface "CategoryController" (FXML) à l'aide de FXMLLoader
        FXMLLoader loader = new FXMLLoader(getClass().getResource("category.fxml"));
        Parent categoryRoot = loader.load();

        // Configurer la scène avec l'interface "CategoryController" chargée
        Scene categoryScene = new Scene(categoryRoot);

        // Définir le titre de la fenêtre (optionnel)
        categoryStage.setTitle("Catégorie");

        // Définir la scène pour le nouveau stage et l'afficher
        categoryStage.setScene(categoryScene);
        categoryStage.show();

        // Fermer la fenêtre actuelle (Ecommerce) si nécessaire
        // ((Node) (event.getSource())).getScene().getWindow().hide();
    } catch (IOException ex) {
        ex.printStackTrace();
